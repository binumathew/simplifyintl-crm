    	...
    	foreach($simList as $sim_data){
    		...
    		if(!$user){
    			...
    			if(!is_null($user->stock_id)){                    
    				...
    				$user_data = ['user_id' => $user->id, 'auth_name' => $username, 'vm_password' => $vm_password, 'vp_password' => $vp_password, 'address' => $address->street, 'city' => $address->city, 'call_settings' => $call_settings, 'state' => $address->country, 'postal_code' => $address->postal_code, 'ip_address' => $ip_address, 'user_platform' => 'AVM', 'register_status' => 0];
