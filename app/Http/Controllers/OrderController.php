        ...
        foreach($request->product as $key => $quantity){
            ...
            if($quantity){                
                if(Cart::whereIn('id', $cartIds)->where('category_id', $plan->id)->exists()){
                    $cart = Cart::whereIn('id', $cartIds)->where('category_id', $plan->id)->update(['sim_count' => 1, 'item_count' => $quantity, 'amount' => $quantity * $plan->sell_price,'promocode' => $promo]);
                }else{
                    $cart = Cart::create(['category' => 'plan', 'category_id' => $plan->id, 'provider' => $plan->sim_provider->id, 'sim_count' => 1, 'item_count' => $quantity, 'amount' => $quantity * $plan->sell_price, 'promocode' => $promo, 'user_id' => $user_id]);
