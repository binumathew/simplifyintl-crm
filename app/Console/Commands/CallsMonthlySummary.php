...
class CallsMonthlySummary extends Command
{
    ...
    public function handle()
    {
        ...
        if($task){
            $currday = Carbon::now()->subMonth(3); //Carbon::parse('2020-09-01')->subMonth();//
            $year = $currday->year;
            $month = $currday->month;
    ...
    private function updateCalllogs($localfile){
        try{
            ...
            $start_month = Carbon::now()->subMonth(3)->format('Y-m-01'); //Carbon::parse('2020-09-01')->subMonth(1)->format('Y-m-02');
            ...
            $i_cdr =  Carbon::now()->subMonth(3)->format('ym').str_pad(1, 9, '0', STR_PAD_LEFT);
