...
class PlanSubscription extends Command
{
    ...
    protected $description = 'Renew User Subscription Every billing cycle';
    protected $schedule;
    ...
    public function handle(){
            try{
                if(ScheduledTask::where(['command' => $this->signature, 'status' => 1])->exists()){
                    ...
                    $customer_id = Utils::settings('mvno_customer_id');
                    $mvno_key = Utils::settings('bundle_mvno_key');

                    $getuserid = DB::table("auto_plan")->select(
                                'user_id',
                                'gateway',
                                'transaction_id',
