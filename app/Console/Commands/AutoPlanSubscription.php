    ...
    public function handle()
    {
        try{
            if(ScheduledTask::where(['command' => $this->signature, 'status' => 1])->exists()){  
                ...
                // $curr_day = Carbon::parse('2020-11-01')->format('Y-m-d');           
                ...
                if($getuserid->isNotEmpty()) { //check for user exists                    
                    foreach ($getuserid as $userdetails) {
                        ...
                        if($rsponse_status === 'SUCCESS' || $rsponse_status === 'SUCCESSWITHWARNING') {                        
                            ...
                            $s_msg = isset($response['L_SHORTMESSAGE0'])?$response['L_SHORTMESSAGE0']:'';
                            $l_msg = isset($response['L_LONGMESSAGE0'])?$response['L_LONGMESSAGE0']:'';
                            ...
                            foreach($autoplanid as $autoid){     
                                ...
                                $payment = ['transaction_id' => '', 'total_amount' => $plans->total_amount, 'payment_method' => $userdetails->gateway, 'payment_for' => 'Plan Monthly Subscription', 'user_id' => $user_id, 'amount' => $plans->amount, 'tax_amount' => $plans->tax, 'description' => 'Subscription Error :'.$l_msg.', ntfy_id:'.$notification->id, 'status' => '0', 'buy_price' => '0'];
                                $paymentid = UserPayment::insertGetId($payment); 
                            ...
                            $msg = 'Hi '. $user->first_name.' Our attempt to process subscription failed. Please contact '.config('settings.app_name').' support ASAP on 0800 108 1810 or update card, to avoid loss of service. Thanks';
