...
use App\Jobs\Supplier\Telna\Location\TelnaLocationJob;

class TelnaLocation extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'telna:location';
     ...
     * @var string
     */
    protected $description = 'Telna Location';
    ...
    private function getfileSFTP($remotePath){
        ...
        try {
            ...
            $lastrun        = strtotime($getprovider->location_lastrun);
        ...
        if($modified > 0 ){
            DB::table('tbl_providers')->whereId($getprovider->id)
                        ->update(['location_lastrun' => date('Y-m-d H:i:s',$modified)]);
        }     
        return true;
    }
        ...
        while ($csvLine = fgetcsv($file, 1000, ",")) {
            if($skipheader){ $skipheader = false; continue;}
                ...
                if(count($cdrData) == 1000){
                    TelnaLocationJob::dispatch($cdrData);
                    $cdrData = [];
                }
            }
        }
